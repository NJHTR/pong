...
| --- | --- | --- | --- |
| [ ] Scope frozen | `ACCEPTED; CLEAN FREEZE PENDING` | Windows x86_64/NTFS and Linux x86_64/ext4 required; macOS GitHub-hosted qualification only with filesystem `unknown` | `M1_OWNER_DECISION_RECORD.md`, `M1_SCOPE_FREEZE.md` |
| [ ] Fault scope frozen | `ACCEPTED; CLEAN FREEZE PENDING` | FI-03 deferred to M2+; FI-07/FI-14 bounded to Linux `tmpfs`; FI-13 Windows NTFS and Linux ext4 | `M1_FI03_DEFERMENT.md`, `M1_FAULT_SCOPE_DECISION.md`, retained fault artifacts |
| [ ] Property policy frozen | `ACCEPTED; CLEAN FREEZE PENDING` | Canonical deterministic 10,000 cases/property plus cross-platform qualification; no 10,000-by-platform multiplication | `M1_PROPERTY_ACCEPTANCE_SCOPE.md`, retained Windows/Linux corpus records |
...
| [ ] Traceability points to candidate | `BLOCKED` | Traceability identifies evidence baseline `6cb62fb...` and Run #5; Owner approval is now `ACCEPTED`, but records `working_tree=DIRTY`, no tag, and no final committed candidate | `artifacts/m1-release-evidence/traceability/release-traceability.json` |
| [ ] CI Run #5 verified | `PASS` | Run `33145714975`, Linux/macOS GitHub-hosted jobs, 13/13 recorded commands per job exited `0` | `artifacts/m1-release-evidence/platform/github-actions-run-33145714975/download-metadata.json` |
| [ ] Working tree ready for final freeze | `NOT_READY` | `git status --short` reports existing uncommitted changes; no clean candidate exists | `git status --short`, `M1_FINAL_CANDIDATE_READINESS.md` |
...
| Item | Value |
| --- | --- |
| HEAD | `6cb62fb455e92ab731a4bb5233856d10c1f1ce93` |
| Branch | `dev` |
